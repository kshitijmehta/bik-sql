-- PROCEDURE: public.spprodsubcateginsertupdatedelete(integer, integer, character varying, text, integer)

-- DROP PROCEDURE public.spprodsubcateginsertupdatedelete(integer, integer, character varying, text, integer);

CREATE OR REPLACE PROCEDURE public.spprodsubcateginsertupdatedelete(
	_ser integer,
	_pcid integer,
...
BEGIN
	IF _ser=1 THEN -- INSERT
		...
		INSERT INTO product_sub_category (prod_category_id, prod_subcateg_name, prod_subcateg_desc)
	...
	ELSEIF _ser=2 THEN --UPDATE
		...
		UPDATE product_sub_category SET
		 ...
		 prod__subcateg_name = _name,
		 prod_subcateg_desc = _desc
